...
| `CBM_GIT_TRACKED_ONLY` | `false` | Set to `1` or `true` for a Git-bound trust snapshot (Git 2.31+ required). The effective mode is always `full` (even if a caller requests `fast`/`moderate`): source files plus tracked graph-shaping manifests are SHA-256 bound, mutable ignore/user config is disabled, and any material/HEAD/branch change triggers a clean FULL rebuild; an identical snapshot is a no-op. This adds full-corpus hashing I/O and fails closed on unresolved or unstable inputs. |
| `CBM_READ_ONLY` | `false` | Set to `1` or `true` before process startup to serve only sealed, existing indexes. Mutating tools and background indexing/watch work are disabled; permitted tools open standalone DELETE-journal databases without creating SQLite sidecars. Pair with `CBM_GIT_TRACKED_ONLY=1` to reject stale source/HEAD/branch snapshots before returning graph data. |
...
`CBM_GIT_TRACKED_ONLY` and `CBM_READ_ONLY` are process-wide in the shared daemon. For strict serving, first run an explicit trusted index with `CBM_GIT_TRACKED_ONLY=1` and read-only mode off. Close every daemon-backed session, then restart with both flags set. The server requires a sealed DELETE-journal generation and fails closed when the database has WAL/SHM/journal sidecars, provenance is incomplete, or the repository snapshot is stale or changes during a request. To refresh, stop the read-only daemon, reindex explicitly with read-only mode off, and restart it.

`CBM_READ_ONLY` prevents MCP tools from mutating indexes or cache databases; it does not suppress explicitly configured diagnostics, stderr/daemon logging, or unrelated operating-system temporary activity. Trusted freshness checks hash the tracked corpus at request boundaries, so strict serving trades additional read I/O for verifiable answers.

The trusted auxiliary corpus currently includes tracked `package.json`, `go.mod`, `Cargo.toml`, `pyproject.toml`, `composer.json`, `pubspec.yaml`, `pom.xml`, `build.gradle[.kts]`, `mix.exs`, `tsconfig.json`, `jsconfig.json`, and `*.gemspec` files. Untracked copies are ignored.
