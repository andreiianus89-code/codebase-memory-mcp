...
if [ -f "$MCP_FILE" ]; then
    ...
    # Update this when legitimate reads are added. 17 reads audited as of the
    # trusted-snapshot work — all path-contained, trusted-root, or transport
    # reads, with no unbounded filesystem read surface.
    EXPECTED_MAX=17
